...
import { schedaInCorso } from './sfida/progresso.js';
...
const daRiprendere = schedaInCorso(id);                       // letto PRIMA di avviare: la prima scheda mostrata sovrascrive il segnalibro
const player = avviaLezione(document.getElementById('lezione'), lezione);
if (q.get('scheda') !== null) player.vai(Number(q.get('scheda')));
else if (q.get('riprendi') !== null && daRiprendere > 0 && daRiprendere < lezione.schede.length) player.vai(daRiprendere);   // dal tasto Continua della home: si riparte dalla scheda dove si era
